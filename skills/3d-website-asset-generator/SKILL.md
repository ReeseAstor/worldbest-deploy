...
5. Open the file in the browser:
   ...
   - Otherwise, use the appropriate platform command to open the file directly:
...
| HTML page doesn't open | Fall back to the platform command (`start` on Windows, `open` on macOS, `xdg-open` on Linux), or tell user the file path to open manually |
