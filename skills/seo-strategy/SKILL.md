...
Save the HTML file next to the user's article (same directory), named `seo-report-{{URL_SLUG}}.html`. Then open it in the user's browser using the appropriate platform command and tell the user where the file is saved:
...
2. Open the file in the user's browser using the appropriate platform command:
   ```
   # Windows
   start [path-to-file]
   # macOS
